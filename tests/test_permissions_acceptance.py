...
from nova.core.permissions import PermissionDecision
from nova.features.sandbox.config import SandboxConfig
...
def test_plan_mode_switches_tool_profile_and_allows_only_active_plan_file(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            '<tool name="write_file" path=".nova/plans/v3-plan.md"><content># Plan\n- Gate 1\n</content></tool>',
    ...
    assert plan_path == ".nova/plans/v3-plan.md"
    ...
    assert (
        rejected
        == "error: plan mode can only write the active plan artifact (.nova/plans/v3-plan.md)"
    )
    ...
    assert agent.active_tool_profile.name == "default"
    assert (
        (tmp_path / ".nova" / "plans" / "v3-plan.md")
...
def test_repeated_plan_mode_denial_is_blocked_before_hitting_step_limit(tmp_path):
    ...
    agent = build_agent(
        ...
        [
            ...
            bad_call,
            bad_call,
            '<tool name="write_file" path=".nova/plans/repeat-plan.md"><content># Plan\n- Retry stopped.\n</content></tool>',
            "<final>Plan ready.</final>",
        ],
        max_steps=6,
    )
...
def test_plan_mode_does_not_allow_retargeting_active_plan_with_enter_tool(tmp_path):
    ...
    assert agent.plan_mode.plan_path == ".nova/plans/original-plan.md"
...
def test_plan_mode_rejects_arbitrary_workspace_plan_path(tmp_path):
    ...
    assert "plan path must stay under .nova/plans/" in rejected
...
def test_repl_plan_command_reports_bad_plan_path_without_crashing(tmp_path):
    ...
    handled, should_exit, output = handle_repl_command(
        agent, "/plan auth .nova/plans/../escape.md"
    )

    assert handled is True
    assert should_exit is False
    assert output == "error: plan path must stay under .nova/plans/"
    assert agent.runtime_mode == "default"
