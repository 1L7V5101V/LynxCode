...
Nova 的证据层有三个层次：session 让系统能继续工作，run artifacts 让一次请求能被复盘，evaluation 让版本之间能比较。没有这三层，agent 项目很容易变成演示能跑、出了问题说不清。

![恢复、复盘和证明](assets/08-session-run-evaluation.png)

## SessionStore 管可恢复会话

`nova/core/session_store.py` 负责 `.nova/sessions/<id>.json` 和 `.events.jsonl` 路径。session JSON 保存的是会话级状态：
...
Claude Code 对这层更激进。它会在用户消息进入 query loop 前先写 transcript，防止进程在 API 返回前被杀时无法 resume。Nova 目前也有 session 和 event 文件，但中途持久化策略没有 Claude Code 那么细。
...
`nova/core/run_store.py` 每次 `ask()` 开一个 run 目录：
...
`nova/evaluation/evaluator.py` 用固定 benchmark、fixture repo 和 `ScriptedModelClient` 跑 deterministic 任务。它验证 runtime 行为是否稳定，不把模型能力混进来。
...
`metrics.py` 再从 benchmark artifact 和 `.nova/runs/` 聚合：
...
Nova 已经有很清楚的本地证据链。它可以回答三类问题：这次任务为什么停下，哪个工具改了什么，某个 runtime 改动有没有破坏固定 benchmark。
...
Nova v3 的 session/run/evaluation 三层就是为这些问题服务。
...
Nova 的 `ScriptedModelClient` 很关键，因为它让测试可以控制模型输出，从而验证 runtime：
...
Human scenario gate 则补上另一层：从真实 CLI/REPL/provider 路径驱动 Nova，再读 artifacts 做验收。它证明系统不是只在 import-level 单测里工作。

### 与托管 agent 生命周期的对应

Managed Agents 把 session、event stream、outcomes、memory store、dream lifecycle 都做成 API 资源。这说明 agent 的“运行过程”本身就是产品对象。

Nova 是本地项目，但也应该保留同样的观念：
...
这也是为什么 `.nova/runs` 和 `.nova/sessions` 不只是 debug 文件夹，而是系统证据层。
