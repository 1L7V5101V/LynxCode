#!/usr/bin/env python3
"""Run business-shaped Nova dogfood scenarios against a real provider."""
...
from nova.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient  # noqa: E402
...
def render_markdown(summary):
    ...
    lines = [
        "# Nova Business Scenario Dogfood",
        "",
...
def _build_client_factory(*, config_path=None, provider=None, model=None, base_url=None, api_key=None):
    ...
    if not config.api_key:
        raise ValueError(f"provider {config.name!r} has no api key; configure .nova.toml or pass --api-key")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run Nova business scenario dogfood against a real provider.")
    parser.add_argument("--output-dir", default="/tmp/nova-business-scenario-dogfood", help="Directory for workspaces and summary artifacts.")
    parser.add_argument("--config", default=None, help="Path to a Nova TOML config file.")
    ...
    parser.add_argument("--max-steps", type=int, default=8, help="Max Nova steps per scenario turn.")
