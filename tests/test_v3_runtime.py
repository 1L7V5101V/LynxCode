...
from nova import Engine, Nova, SessionEventBus, SessionStore, WorkspaceContext
...
def test_engine_drives_real_session_and_persists_event_timeline(tmp_path):
    ...
    assert (
        ...
        == tmp_path / ".nova" / "sessions" / f"{agent.session['id']}.events.jsonl"
    )
...
def test_plan_mode_tools_enter_and_exit_runtime_mode(tmp_path):
    ...
    assert ".nova/plans/refactor-auth-plan.md" in entered
    assert agent.runtime_mode == "plan"
...
def test_plan_path_accepts_absolute_path_inside_workspace(tmp_path):
    """模型偶尔给绝对路径，如 /Users/u/repo/.nova/plans/foo —— 自动相对化，
    不应该让 agent 多走一次重试。"""
    from nova.core.plan_mode import _plan_path

    assert (
        _plan_path("Student Mgmt", "/Users/u/repo/.nova/plans/student-mgmt.md")
        == ".nova/plans/student-mgmt.md"
    )
    assert (
        _plan_path("X", "./.nova/plans/x-plan.md") == ".nova/plans/x-plan.md"
    )
    # 真正越界的还是要拒
    import pytest
