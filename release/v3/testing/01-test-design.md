...
v3 相对 `main` 不是一个小补丁，而是把 Nova 从一个单层 CLI agent 推到了本地 coding agent runtime harness：入口层多了 TUI/REPL/one-shot 的选择，控制层拆出了 Engine、run artifacts、session event bus、plan mode、worker manager，能力层多了 skills、todo、sandbox、provider profile、分层记忆和 auto-dream。

所以这批测试不应该继续只走 `Nova(...)` 单元对象。它应该模拟人从终端打开 Nova、输入自然语言或 slash command、确认审批、退出、恢复 session，然后再检查 `.nova/sessions/`、`.nova/runs/`、工作区文件和终端/TUI 展示。
...
设计 50 个端到端场景，每个场景都从用户入口驱动 Nova：

- Computer Use 操作 macOS Terminal / iTerm / Codex 内置终端里的 TUI。
- PTY/expect 操作 `uv run nova --repl`，模拟键盘输入、回车、等待输出。
- one-shot CLI 操作 `uv run nova "<prompt>"`，模拟用户直接执行一次任务。
- 事后验证只读检查 `.nova/sessions/*.json`、`.nova/sessions/*.events.jsonl`、`.nova/runs/*/{task_state.json,trace.jsonl,report.json}` 和工作区文件。
...
|---|---|---|
| Engine 与 run artifacts | `nova/core/engine.py`, `nova/core/runtime.py`, `nova/core/run_store.py` | 一次任务从输入到 final 全程写 session events、trace、report |
| TUI / REPL / one-shot 入口 | `nova/cli.py`, `nova/tui/*` | TTY 默认 TUI、`--repl` 退回普通 REPL、prompt 参数走 one-shot |
| Slash commands | `nova/commands/slash.py` | `/help`、`/session`、`/usage`、`/context`、`/model`、`/history`、`/resume` 等 |
| Plan mode | `nova/core/plan_mode.py`, `nova/tools/plan.py` | 只能写 active plan artifact，未写计划不能 final，退出后恢复 default |
| Tool policy / permission | `nova/core/tool_policy.py`, `nova/core/permissions.py`, `nova/core/tool_executor.py` | read-before-write、shell search 拒绝、审批 ask/auto/never |
| Sandbox | `nova/features/sandbox/*` | `required` 缺 backend fail closed，`best_effort` degrade 可见 |
| Skills | `nova/features/skills*.py`, `docs/skills.md` | 内置 skill、项目 skill、参数替换、allowed-tools、fork、prompt-only |
| Subagent / worker | `nova/core/worker_*.py`, `nova/tools/agents.py` | Explore 只读、worker 写 scope、续接、停止、计划模式禁止写 worker |
| Todo ledger | `nova/core/todo_ledger.py`, `nova/tools/todos.py` | todo_add/update/list 写入 report 和 prompt |
| Memory / auto-dream | `nova/features/memory.py`, `docs/memory.md` | `/remember`、`/dream`、topic 文件、secret-shaped 内容拒绝、auto-dream gate |
| Context governance | `nova/core/context_manager.py`, `nova/core/compact.py` | `/compact`、自动 compact、context usage 进 report |
| Provider profiles | `nova/config/__init__.py`, `nova/providers/clients.py` | OpenAI-compatible、Anthropic-compatible、DeepSeek profile、usage/cache/error 元数据 |
| Safety / redaction | `nova/core/runtime_secrets.py`, `nova/core/workspace.py` | path traversal 拒绝、symlink 越界拒绝、trace/report 脱敏 |
...
scenario.yaml
  -> 创建临时 workspace
  -> 启动 uv run nova --cwd <workspace> --repl 或 --tui
  -> Computer Use / PTY 逐行输入
  -> 等待 final / prompt 回来
  -> 退出 Nova
  -> 只读验证工作区和 .nova artifacts
```

测试入口必须像人一样使用 Nova。验证器可以直接读文件，但不能绕过入口去 import `Nova` 调 runtime 方法。
...
| 依赖 | 用途 | 降级策略 |
|---|---|---|
| `uv` | 启动本地 Nova、临时 pytest | 没有 `uv` 时场景标记 environment failure |
| Python 3.10+ | Nova 运行要求 | 低版本直接失败 |
...
3. Runtime 证据：`.nova/runs/<run_id>/report.json`、`trace.jsonl`、`task_state.json` 存在且字段正确。
4. Session 证据：`.nova/sessions/<session_id>.json` 和 `.events.jsonl` 记录关键事件。
...
| R03 | 发布就绪审查报告 | PTY REPL + project skill | 创建 `.nova/skills/release/SKILL.md`，用户输入 `/release billing-api` | project skill、allowed-tools、write_file、skill_invoked/skill_completed | `reports/release-readiness.md` 写出；events 有 skill_invoked/skill_completed；业务文件未被改 |
| R04 | 线上事故续接修复 | PTY REPL 两次启动 | 第一次让 Nova 读事故测试并 `todo_add` 后退出；第二次 `uv run nova --resume latest --repl` 输入“继续修复并跑测试” | `/resume`、session persistence、todo ledger、patch_file、run artifacts | 同一 session id；todo_1 done；`classify_latency` 修复；第二次 report 有 todos 和 todo_changes |
...
| ID | 场景 | Driver | 用户动作 | 覆盖 v3 改动 | 验收证据 |
|---|---|---|---|---|---|
| S06 | TTY 默认进入 TUI | Computer Use | 在 Terminal 执行 `uv run nova --cwd <workspace>` | `interaction_mode` 默认 TUI | 屏幕有 Nova TUI status bar；没有直接执行 one-shot |
| S07 | `--repl` 进入普通 REPL | PTY | 执行 `uv run nova --cwd <workspace> --repl`，输入 `/help` | REPL fallback、slash help | 输出含 `Commands:`、`/memory`、`/subagent` |
| S08 | prompt 参数走 one-shot | one-shot CLI | 执行 `uv run nova --cwd <workspace> "列出 README 摘要"` | one-shot 入口、无交互 session | 命令退出码为 0；report status completed；没有等待 REPL 输入 |
| S09 | piped stdin 使用 REPL | PTY | `printf '/help\n/exit\n' | uv run nova --cwd <workspace> --repl` | 非 TTY 行为 | 输出命令列表并正常退出 |
...
| S13 | `/model` 只改当前 runtime | PTY REPL | 输入 `/model gpt-test-local`，再 `/model` | runtime-only model switch | 输出 `model: gpt-test-local`；workspace 没有新增 `.nova.toml` |
...
| S16 | 未写计划不能 final | PTY REPL | `/plan cache` 后要求“只口头说计划完成，不写文件” | `plan_mode.can_finish()` final gate | history 有 runtime_notice；最终必须写 `.nova/plans/cache-plan.md` 才完成 |
| S17 | absolute plan path 自动归一 | PTY REPL | `/plan student` 后提示写绝对路径 `<workspace>/.nova/plans/student-plan.md` | plan path normalize | 实际写入 `.nova/plans/student-plan.md`；未触发越界错误 |
| S18 | 越界 plan path 被拒 | PTY REPL | 让模型尝试写 `.nova/plans/../escape.md` | plan path traversal guard | 输出 `plan path must stay`；workspace 外无文件 |
...
| ID | 场景 | Driver | 用户动作 | 覆盖 v3 改动 | 验收证据 |
|---|---|---|---|---|---|
| S21 | 改文件前必须先读 | one-shot CLI | 要求“把 README 的 world 改成 nova”，但提示不能先读 | prior_read_required | 第一次 patch 被拒并提示 read_file；随后读文件再 patch；report 有 rejected reminder |
...
| S26 | 长 shell 输出落 artifact | one-shot CLI | 让 Nova 运行输出 6000 字符的命令 | output clipping、full output artifact | tool history 被裁剪；report/trace 指向 full_output_artifact；artifact 文件含完整输出 |
| S27 | approval `never` 拒绝 risky tool | PTY REPL | `uv run nova --approval never --repl` 后要求写文件 | PermissionChecker single gate | 输出 approval denied；文件未写；events 有 permission_decision deny |
| S28 | sandbox required 缺 backend fail closed | one-shot CLI | macOS 上执行 `uv run nova --sandbox required "运行 echo hi"` | sandbox fail closed | 输出 sandbox unavailable；report status failed 或 tool_failed；没有静默直跑 |
| S29 | sandbox best_effort degrade 可见 | one-shot CLI | macOS 上执行 `uv run nova --sandbox best_effort "运行 echo hi"` | sandbox degrade event | 命令成功输出 hi；events 有 sandbox_unavailable |
...
| S32 | 项目 skill 参数替换 | PTY REPL | 创建 `.nova/skills/deploy/SKILL.md`，输入 `/deploy staging` | `$ARGUMENTS`、`${NOVA_SKILL_DIR}` | final 提到 staging；events 有 skill_invoked；prompt 含 skill dir |
...
| ID | 场景 | Driver | 用户动作 | 覆盖 v3 改动 | 验收证据 |
|---|---|---|---|---|---|
| S43 | `/remember` 写 daily log | PTY REPL | 输入 `/remember 这个项目用 pytest，不用 unittest` | daily log、memory event | `.nova/memory/logs/YYYY/MM/*.md` 有内容；events 有 memory_note_appended |
| S44 | `/dream` 写 topic 和 MEMORY.md | PTY REPL + live/model | 先 `/remember` 三条稳定事实，再输入 `/dream` | dream child run、memory write scope | `.nova/memory/MEMORY.md` 和 `topics/*.md` 非空；不会写 memory 外文件 |
