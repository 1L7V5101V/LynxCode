...
Skill 是一段写在 markdown 文件里的可复用 prompt，可以通过 `/skill-name [args]` 调用。nova 把它展开成一次普通 session 请求，沿用同一套工具、审批和事件链路。
...
1. **内置 skill** — nova 自带
2. **用户 skill** — `~/.nova/skills/<name>/SKILL.md`
3. **项目 skill** — `<repo>/skills/<name>/SKILL.md` 或 `<repo>/.nova/skills/<name>/SKILL.md`

## 自定义一个 skill

最小例子，新建 `~/.nova/skills/deploy/SKILL.md`：
...
`$ARGUMENTS` 会被替换为 `staging`。`${NOVA_SKILL_DIR}` 会被替换为 skill 文件所在目录的绝对路径。
