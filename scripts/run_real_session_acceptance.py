#!/usr/bin/env python3
"""Run deterministic real-session acceptance scenarios for Nova."""
...
from nova.testing import ScriptedModelClient  # noqa: E402
from nova import Nova, SessionStore, WorkspaceContext  # noqa: E402
from nova.config import resolve_provider_config  # noqa: E402
from nova.features.skills_runtime import invoke_skill  # noqa: E402
from nova.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient, ProviderError  # noqa: E402

SUMMARY_JSON = "gate8-real-session-acceptance.json"
SUMMARY_MARKDOWN = "gate8-real-session-acceptance.md"
LIVE_ENV_FLAG = "NOVA_ACCEPTANCE_LIVE"
...
def _scenario_plan_todo_explore(output_dir, workspace):
    ...
    agent = _build_agent(
        ...
        [
            ...
            '<tool name="write_file" path=".nova/plans/gate8-plan.md"><content># Gate8 Plan\n- Evidence harness\n</content></tool>',
    ...
    return _finalize(
        ...
        checks=[
            ...
            _check("plan_file", (workspace / ".nova" / "plans" / "gate8-plan.md").is_file()),
...
def _scenario_skill_inline(output_dir, workspace):
    ...
    skill_dir = workspace / ".nova" / "skills" / "evidence"
...
def _scenario_resume_continuation(output_dir, workspace):
    ...
    store = SessionStore(workspace / ".nova" / "sessions")
    first = Nova(
...
def _scenario_security_rejection(output_dir, workspace):
    ...
    old_secret = os.environ.get("NOVA_ACCEPTANCE_SECRET")
    os.environ["NOVA_ACCEPTANCE_SECRET"] = "nova-secret-value-123"
    agent = _build_agent(
        workspace,
        [
            ...
            '<tool>{"name":"run_shell","args":{"command":"echo $NOVA_ACCEPTANCE_SECRET","timeout":5}}</tool>',
    ...
    try:
        ...
        return _finalize(
            ...
            checks=[
                ...
                _check("secret_redacted", "nova-secret-value-123" not in trace_text),
            ],
        )
    finally:
        if old_secret is None:
            os.environ.pop("NOVA_ACCEPTANCE_SECRET", None)
        else:
            os.environ["NOVA_ACCEPTANCE_SECRET"] = old_secret
...
def _scenario_provider_error_recovery(output_dir, workspace):
    ...
    agent = Nova(
        ...
        workspace=_scenario_workspace(workspace),
        session_store=SessionStore(workspace / ".nova" / "sessions"),
        approval_policy="auto",
        max_steps=2,
    )
...
def _scenario_live_provider_smoke(output_dir, workspace, include_live=None):
    ...
    agent = Nova(
        ...
        max_steps=1,
        max_new_tokens=64,
        secret_env_names=["NOVA_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY"],
    )
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run Nova Gate8 deterministic real-session acceptance scenarios.")
