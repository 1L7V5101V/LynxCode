...
REPO="${NOVA_REPO:-https://github.com/martin-los/nova.git}"
INSTALL_DIR="${NOVA_INSTALL_DIR:-$HOME/.nova-agent}"
BRANCH="${NOVA_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[nova]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[nova]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[nova]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[nova] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
