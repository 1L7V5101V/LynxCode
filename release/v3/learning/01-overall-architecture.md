# 总体架构：Nova 是本地 coding agent harness，不是聊天壳

Nova 的主线可以先压成一句话：它把模型、工具、上下文、记忆、权限、持久化和评测包成一条本地执行链，让模型能在真实代码仓库里连续工作，而不是只回答一次问题。

![Nova 总体架构](assets/01-overall-architecture.png)
...
控制面决定任务怎么往前推。`nova/cli.py` 把命令行参数、provider 配置、工作区和 session store 装成一个 `Nova` 实例；`nova/core/runtime.py` 持有运行时对象图；`nova/core/engine.py` 负责 turn 级主循环；`nova/tools/` 和 `nova/providers/` 分别接外部动作和模型后端。

状态面决定系统能不能接着干。`SessionStore` 保存会话，`LayeredMemory` 保存工作记忆，`.nova/memory/` 保存 durable memory，`PlanModeController` 和 `TodoLedger` 保存当前计划和任务 ledger，`WorkerManager` 保存子 agent 状态。
...
第一，`Runtime` 和 `Engine` 分开。`Nova` 负责持有工作区、session、memory、tools、workers、permissions、context manager；`Engine` 负责把一个用户请求推进成模型调用、工具调用、最终回答和 run 工件。这样读代码时不用把对象装配和循环推进混在一起。

第二，工具执行和模型输出解析都被拆出去。`core/model_output.py` 只处理 Nova 的文本协议，把模型返回解析成 `tool/tools/final/retry`；`core/tool_executor.py` 只处理工具调用边界，包括参数校验、权限、policy、重复调用、工作区 diff 和输出截断。

## 为什么不是最小 ReAct

最小 agent 教学实现通常会把智能体拆成四块：context 管消息，LLM 管模型，tools 管工具，agent 管 ReAct 循环。这个拆法适合教学，因为它先把最小闭环讲清楚。

Nova 的问题更靠后一层。它关心的不只是 LLM 能不能调用工具，还包括这些问题：
...
所以 Nova 的模块数量更多，原因是它开始处理最小 agent 之外的工程问题，不是单纯做功能扩张。
...
Nova 对标它时，不能用功能数量比较。更有价值的比较是系统分层：
...
Nova 的优势是小而可读，模块边界容易讲清楚。Claude Code 的优势是产品级覆盖面和长期任务可靠性更完整。学习时可以拿 Claude Code 做参照，但不要把它的全部复杂度搬进 Nova。

## 现在的架构判断

Nova 已经形成了一个可评审的本地 harness：
...
## 设计文档级补充：Nova v3 的系统边界

这一篇如果按设计文档来读，核心问题不是“Nova 有哪些模块”，而是：一个本地 coding agent 从演示程序变成 runtime harness，需要把哪些不稳定因素收进系统边界。
...
Nova v3 的架构边界可以压成四句话：

- `Nova` 是运行现场，不是模型 wrapper。
...
Nova v3 的目标不是复制完整商业 coding agent，而是建立一个可解释、可复盘、可验证的本地 harness。它要满足五个工程约束：
...
这也是为什么 Nova 不把所有逻辑塞进 `runtime.py`。一旦系统开始负责权限、记忆、worker、compact、evidence，单文件主循环会变成不可审计的状态泥团。
...
成熟 coding agent 往往会把系统拆成 query engine、tool protocol、permission context、command registry、memory subsystem、transcript store、UI renderer、telemetry 和 experiment control。Nova v3 没有同等体量，但已经有相同的骨架：

| 成熟系统问题 | Nova v3 对应实现 | 当前缺口 |
| --- | --- | --- |
...
这个对照说明 Nova 的取舍：先做清楚边界，再扩大能力面。

### 失败模式

Nova 作为 harness，最危险的失败不是“回答不好”，而是下面这些系统性失败：
...
3. **Tool protocol split**：工具定义、校验、权限提示、执行、结果预算进一步模块化，但保持 `Nova.run_tool()` 是总闸口。
...
面试里可以这样收束：Nova 的价值不是工具数量，而是把本地 coding agent 最容易失控的几层变成显式控制面和证据面。它让模型能行动，但行动必须被约束、记录、恢复和验证。
