...
`scripts/run_v3_human_scenario_gate.py` 是 Nova v3 真人场景 runner。它刻意走 Nova 的公开进程入口：

- one-shot CLI：`uv run nova --cwd <workspace> "<prompt>"`
- REPL：`uv run nova --cwd <workspace> --repl`
- PTY-style stdin：模拟用户逐行输入 slash command
- TTY smoke：验证默认 TTY 入口能进入 TUI

runner 不 import `Nova`，也不直接调用 runtime 方法。它只创建临时 workspace、启动 Nova 进程、收集 stdout/stderr，然后读取 Nova 自己写出的 `.nova` artifacts。
...
```text
/tmp/nova-v3-human-scenarios/<YYYYMMDD-HHMMSS>
```

macOS 上通常会显示为：

```text
/private/tmp/nova-v3-human-scenarios/<YYYYMMDD-HHMMSS>
```
...
每个 scenario workspace 都会先 `git init -q`。这是必要的：Nova 会根据 git root 识别 workspace，如果把测试目录放在 Nova repo 内且不初始化独立 git root，Nova 会向上找到真实项目根目录，导致测试污染 repo。
...
```bash
uv run python scripts/run_v3_human_scenario_gate.py --suite full --config /Users/martinlos/code/nova/.nova.toml
```

指定输出目录：

```bash
uv run python scripts/run_v3_human_scenario_gate.py --suite full --output-dir /tmp/nova-v3-human-scenarios/manual-run
```

`--output-dir` 不能位于 `/Users/martinlos/code/nova` 下面。runner 会直接拒绝这种路径。
...
```json
{
  ...
  "suite": "full",
  "provider": "deepseek",
  "output_dir": "/private/tmp/nova-v3-human-scenarios/20260513-170838"
}
```
...
- `checks`：具体验收项
- `commands`：实际执行的 Nova 命令、return code、stdout/stderr 路径
...
`nova/evaluation/run_evidence.py` 提供 `RunEvidence.latest(workspace)`，只从真实 artifacts 取证：

- `.nova/runs/run_*/report.json`
- `.nova/runs/run_*/trace.jsonl`
- `.nova/sessions/*.events.jsonl`
...
3. 打开 `workspaces/<scenario>/.nova/runs/<run_id>/report.json`，看 status、stop_reason、runtime_reminders。
4. 打开 `trace.jsonl`，按 `tool_executed` 搜索 `tool_error_code`、`name`、`result`。
5. 打开 `.nova/sessions/*.events.jsonl`，看 slash command、permission_decision、skill_invoked、worker 事件。
