# Skills、命令、CLI 和 TUI：人怎么驱动 Nova

Nova 的交互层分两条线。CLI/REPL/TUI 负责把人的输入送进 runtime，skills 和 slash commands 负责把常见行为变成可触发的工作流。它们都不是主循环本身，但会直接影响模型看到什么、能做什么。

![人如何驱动 Nova](assets/07-skills-commands-cli-tui.png)
...
`nova/commands/slash.py` 定义命令元信息，`handle_repl_command()` 执行具体行为。常用命令包括：
...
`nova/features/skills.py` 的 skill loader 会从三类路径加载：
...
Nova 支持的 metadata 更丰富，包括 context、allowed_tools、argument_hint、disable_model_invocation、model、paths 等。当前核心路径仍然是 prompt-driven：skill 被选中后变成 prompt 指令，告诉模型如何组合已有工具。
...
Nova 的默认策略是先暴露可用入口，不把所有 skill 正文都塞进 prompt。具体 skill 执行在 `features/skills_runtime.py`，由 slash command `/skill <name> [args]` 或直接 `/skill-name` 路径触发。
...
Nova 目前已经有 progressive disclosure 的雏形，但还没有做到 Claude Code 那种技能发现、工具执行、插件系统、动态目录触发的一整套闭环。

## TUI 是 presentation layer

`nova/tui/app.py` 用 Textual 包了一层 UI。它不重新实现运行时，而是调用同一个 `agent.engine.run_turn()` generator。运行时事件会被映射成：
...
Nova 的交互层应该保持轻。它要展示本地 harness 的关键反馈：当前模型、approval、工具调用、工具结果、worker notification、context usage、session 状态。
...
`nova/cli.py` 不应该实现 agent loop。它的职责是把用户启动参数翻译成 runtime object graph：
...
Nova 的 slash command 不是都给模型看的。它们可以分成两类：
...
Nova 现在已有 progressive disclosure 的雏形：prompt section 先列技能入口，具体 skill 由 runtime invoke 展开。下一步应该把 `allowed-tools` 和 tool profile 接起来，让 skill 不只是提示词，而是可治理 workflow。
...
Nova 不需要一次性复制这些，但可以学习一个原则：所有入口最终都应该落到同一个 runtime event contract。CLI 打印、TUI 渲染、report 生成、测试断言都消费同一条事件流。
