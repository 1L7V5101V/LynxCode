...
Nova 当前最关键的架构变化，是把运行时对象图和 turn 级执行循环分开了。`Nova` 更像运行现场，`Engine` 负责把一次用户请求推进到底。

![Runtime 与 Engine 控制流](assets/02-runtime-engine.png)

## Nova runtime 负责装配现场

`nova/core/runtime.py` 里的 `Nova.__init__()` 把系统运行需要的对象一次性挂好：
...
`nova/core/engine.py` 的 `run_turn()` 是 turn 级状态机。它先创建 `TaskState` 和 run 目录，然后进入循环：
...
## model_output 是 Nova 的文本协议边界

`nova/core/model_output.py` 把模型原始文本解析成四类：
...
Nova 没有直接使用 provider 原生 tool calling，而是定义了 `<tool>...</tool>` 和 `<final>...</final>` 文本协议。这个选择让 OpenAI-compatible、Anthropic-compatible、DeepSeek 走同一条 runtime 逻辑，但代价是模型必须遵守文本格式，malformed response 要靠 retry notice 拉回。

这里可以和最小 agent 实现对照看。最小实现往往直接用模型 API 的原生 tool-use 格式，上下文格式更贴近单一后端。Nova 选择文本协议，是为了 provider 统一和测试可控。

## tool_executor 是动作边界

`nova/core/tool_executor.py` 负责工具执行的统一边界，顺序大概是：
...
Claude Code 的 `QueryEngine.ts` 更像 Nova 的 `Runtime + Engine + CLI/TUI 交界` 的组合。它在 `submitMessage()` 里做了很多 Nova 目前还拆得比较简单的事情：
...
Nova 的设计更轻：一轮模型调用就是 `complete_model()`，没有 streaming block，也没有 SDK message 兼容层。它的好处是读起来直，测试容易写；不足是对长输出、部分流式结果、工具并发和 provider stall 的处理还不够细。

## 当前取舍

Nova 的 runtime 拆法是合理的。`Runtime` 持有状态，`Engine` 推进循环，`model_output` 管协议解析，`tool_executor` 管动作边界，这四块各自只做一件事。
...
- transcript 级中途持久化更激进，Claude Code 已经把用户消息先写 transcript，Nova 现在主要依赖 session/run 写入。

面试里可以这样讲：Nova 的主循环已经是 turn 级状态机。它把 prompt 构建、模型协议、工具边界、checkpoint、memory maintenance 和 run artifacts 都纳入同一条控制流，所以能解释任务为什么继续、为什么停下、停下后留下了什么证据。

## 设计文档级补充：turn loop 的可靠性边界

Nova v3 的 runtime 设计重点，是把“模型输出一次文本”变成“一个可恢复、可审计、可停止的 turn”。这里的核心不是 while 循环，而是状态转移。
...
### 为什么 `Nova` 和 `Engine` 要分开

`Nova` 拥有对象图。它知道 workspace、session、memory、tools、permissions、workers、provider、context manager、run store 在哪里。
...
当前代码里 `Nova.ask()` 只是委派到 `Engine.ask()`，这是正确方向。真正要审查的是 `Engine.run_turn()` 是否保持 turn 状态机的单一入口。
...
Nova v3 区分 `attempts` 和 `tool_steps`。这是一个关键工程判断。
...
Nova 当前用 `<tool>` / `<final>` 文本协议统一不同 provider。这带来两个收益：
...
Nova 当前还偏 blocking：provider 返回完整文本后再解析。这个实现简单，但对长输出、慢模型、部分失败和 UI 实时反馈都不够强。
