...
这个目录把 Nova v3 相对 `main` 的真人使用场景测试整理成一个可以复盘、重跑、继续扩展的测试包。

它不是单元测试说明，也不是 pytest 清单。它记录的是：像真实用户一样从 `uv run nova`、`--repl`、slash command、resume、provider profile、skills、worker、memory 等入口使用 Nova，然后只读检查 `.nova/runs` 和 `.nova/sessions` 产物。
...
| `nova/evaluation/run_evidence.py` | 从真实 `.nova/runs`、`.nova/sessions` 读取证据 |
...
- 场景必须从用户入口驱动 Nova，不能 import `Nova` 直接调 runtime。
- 验证器可以读文件，但只能读 Nova 自己写出的 artifacts 和 scenario workspace。
- 输出目录必须在 repo 外，避免 Nova 向上发现真实 repo root。
- live provider 默认用 DeepSeek，配置来自项目 `.nova.toml`，不把 key 写进文档或产物。
