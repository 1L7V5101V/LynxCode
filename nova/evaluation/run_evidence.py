"""Read-only helpers for Nova run/session evidence.

These helpers intentionally do not import or instantiate the Nova runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.nova/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".nova" / "runs")
        ...
        session_path = _latest_file(workspace / ".nova" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".nova" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
