...
50 个真人使用场景已经用进程级 Nova 入口执行完成，最终结果为：

- Human scenario full suite：`50 passed / 0 failed`
- Runner 输出目录：`/private/tmp/nova-v3-human-scenarios/20260513-170838`
- Summary JSON：`/private/tmp/nova-v3-human-scenarios/20260513-170838/summary.json`
- Summary Markdown：`/private/tmp/nova-v3-human-scenarios/20260513-170838/summary.md`
- 代码验证：`uv run ruff check .` 通过
- 回归测试：`uv run pytest tests -q` -> `224 passed, 2 skipped`

这次不是只改测试代码。过程中暴露出的 runtime 行为问题已经修在产品代码里，runner 只承担“像人一样跑 Nova 并收集证据”的职责。
...
| `nova/evaluation/run_evidence.py` | 从 `.nova/runs`、`.nova/sessions` 读取真实运行证据的复用适配层 |
| `nova/core/tool_repetition.py` | 重复工具调用守卫，按当前用户轮次和工具语义判断 |
| `nova/core/tool_executor.py` | 在 permission / policy 前拦截重复调用，避免重复坏调用打满 step |
| `nova/core/runtime.py` | 接入重复调用守卫，保持 runtime 主体行数预算 |
| `nova/cli.py` | `/plan <topic> <bad_path>` 不再让 REPL 崩溃，改为用户可见错误 |
...
- `workspaces/<scenario>/.nova/runs/<run_id>/report.json`
- `workspaces/<scenario>/.nova/runs/<run_id>/trace.jsonl`
- `workspaces/<scenario>/.nova/sessions/*.events.jsonl`

## 过程中发现的问题与处理

### 1. 场景 workspace 不能放在 Nova repo 内

现象：早期 runner 把 workspace 放在 repo 子目录时，Nova 向上发现 `/Users/martinlos/code/nova/.git`，导致场景文件可能写进真实项目。
...
- 每个 scenario workspace 内部执行 `git init -q`，让 Nova 识别到隔离 repo root。
- runner 拒绝把 `--output-dir` 放在 Nova repo 内。
...
- 逻辑抽到 `nova/core/tool_repetition.py`，避免继续膨胀 `runtime.py`。
...
处理：`handle_repl_command()` 捕获 `ValueError`，返回 `error: plan path must stay under .nova/plans/`。
...
- “没有启动模型 run”不能用 `.nova/runs` 目录是否存在判断，只能看是否存在 `run_*`。
- REPL 输出里 session id 可能带 `nova>` prompt 前缀。

处理：新增 `RunEvidence`，统一从真实 `.nova/runs` 和 `.nova/sessions` 读取证据，runner 不再散落 ad hoc JSON 读取逻辑。
...
```text
{"failed": 0, "output_dir": "/private/tmp/nova-v3-human-scenarios/20260513-170838", "passed": 50, "status": "passed"}
All checks passed!
