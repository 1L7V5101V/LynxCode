...
```
CLI 显式参数 > 环境变量 > 项目 .nova.toml > 全局 ~/.config/nova/config.toml > 代码默认
```
...
```bash
nova                       # 用 toml 里的默认 provider
nova --provider openai     # 临时切换
nova --provider anthropic --model claude-opus-4-6
```
...
| 变量 | 用途 |
|------|------|
| `NOVA_PROVIDER` | 默认 provider |
| `NOVA_API_KEY` / `NOVA_BASE_URL` / `NOVA_MODEL` | 通用 override |
...
兼容历史 `.env`：`NOVA_OPENAI_*` / `NOVA_ANTHROPIC_*` / `NOVA_DEEPSEEK_*` 仍然能用。

## 全局配置

`~/.config/nova/config.toml` 适合放跨项目都用的 provider profile。项目 `.nova.toml` 覆盖它，CLI 参数再覆盖项目。

## CLI 参数

```bash
nova --provider deepseek --model deepseek-v4-pro
nova --api-key sk-... --base-url https://...
nova --max-steps 50 --max-new-tokens 4096
nova --temperature 0.0
nova --approval ask          # ask | auto | never
nova --sandbox best_effort   # off | best_effort | required
nova --no-auto-dream         # 关闭后台 memory 整合
nova --cwd /path/to/repo     # 切换工作目录
nova --resume latest         # 续接上一个 session
nova --config /path/to/custom.toml
```
