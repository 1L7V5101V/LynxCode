...
Nova 的 provider 层目标很克制：runtime 不应该知道某个后端走 `/responses` 还是 `/messages`，不应该自己解析 SSE，也不应该关心 usage 字段叫 `input_tokens` 还是 `prompt_tokens`。runtime 只要一个 `complete()`。

![Provider 配置收口](assets/06-providers-config.png)

## 配置入口

`nova/config/__init__.py` 负责把配置来源收敛成 `ProviderConfig`：

- 默认配置：`openai`、`anthropic`、`deepseek`
- 项目配置：`.nova.toml`
- 全局配置：`~/.config/nova/config.toml`
- 项目 `.env`
- 环境变量
- CLI 参数覆盖
...
`nova/cli.py` 的 `_build_model_client()` 读取 `ProviderConfig` 后，只做一件事：根据 protocol 创建 client。
...
这层设计的好处是，runtime 不关心协议差异。坏处是 Nova 目前没有用 Anthropic 原生 tool use，而是统一走文本协议，所以 provider 的结构化能力没有被充分利用。
...
Claude Code 的 API 层比 Nova 更像可靠性控制面。它有 streaming、usage/cost tracking、retry 分类、prompt cache、预算控制、slow operation、feature flag、模型 override 和 telemetry。`QueryEngine` 还会维护 total usage、permission denials、SDK status 和 session transcript。
...
| 配置 | `.nova.toml`、`.env`、环境变量、CLI | config、managed settings、GrowthBook、feature gates |

## 当前取舍

Nova 的 provider 抽象适合当前阶段：统一接口、统一 metadata、统一错误形状，先让 runtime 不被协议细节污染。
...
第二，`.env` 和 `.nova.toml` 是项目本地入口。这样 release 和测试能复用真实配置路径，而不是靠 shell 临时 export。
...
成熟 agent 很重视 prompt cache，不只是为了省钱，也为了降低长任务延迟。Nova 现在已有 prompt_cache_key / retention 的入口和 provider metadata，但还缺少 cache miss 解释。
...
Managed Agents 的 Dreams 文档把异步 job、usage、status、error、output resource 都作为一等资源暴露。这说明 provider/API 层不只是“发请求”，它还要表达生命周期、成本和失败。Nova 虽然不是托管平台，但 provider 和 run report 也应该尽量保留这些信息。
...
- `.nova.toml`、`.env`、环境变量、CLI override 优先级正确。
