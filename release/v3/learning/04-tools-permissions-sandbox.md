...
Nova 的工具走显式注册，不做动态发现。基础工具在 `BASE_TOOL_SPECS` 里定义，包括：
...
`PermissionChecker.check()` 先看当前 tool profile。Nova 有几种 profile：
...
工具执行前后，Nova 会对 risky 工具做工作区快照，执行后计算：
...
长 shell 输出还会写进 run artifact，只在 prompt 里保留截断摘要。这一点和 Claude Code 的 tool result budget 思路类似，只是 Nova 现在只覆盖 `run_shell`。
...
Nova 的工具系统适合当前体量。它没有 MCP 和几十个工具，但有统一执行边界，这比盲目堆工具更重要。
...
`nova/tools/registry.py` 定义工具能力。它应该回答：
...
Nova 当前 sandbox 只覆盖 `run_shell`，支持 `off/best_effort/required`。在 macOS 上 bubblewrap 不可用，所以它不能作为唯一安全手段。文档和测试里必须把这个边界讲清楚：sandbox 是 defense-in-depth，不是 approval 的替代品。

### 工具结果也是协议

工具返回值不能只是字符串。Nova 需要知道：
...
Nova 当前只做了小集合，但应该保留协议意识。未来增加 MCP、web、notebook、LSP 之前，先把当前工具的协议边界打稳。
