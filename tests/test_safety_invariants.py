...
from nova.core.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = nova_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = nova_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = nova_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = nova_cli.build_agent(args)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("NOVA_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("nova.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = nova_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = nova_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["NOVA_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "NOVA_CUSTOM_SECRET": "custom-secret-value",
            "NOVA_SECRET_ENV_NAMES": "NOVA_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("nova.cli.OpenAICompatibleModelClient", DummyModelClient):
        args = nova_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = nova_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["NOVA_CUSTOM_SECRET"]
...
def test_bound_tool_methods_call_tools_module(tmp_path):
    ...
    with patch("nova.tools.registry.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "nova.core.runtime"
