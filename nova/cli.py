...
这个模块负责把“用户怎么启动 nova”翻译成 runtime 能理解的对象：
...
SECRET_ENV_NAMES_VAR = "NOVA_SECRET_ENV_NAMES"
...
def build_agent(args):
    ...
    - 输出：一个新的 `Nova`，或一个从旧 session 恢复出来的 `Nova`
    ...
    store = SessionStore(workspace.repo_root + "/.nova/sessions")
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--config", default=None, help="Path to a Nova TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .nova/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
...
def main(argv=None):
    ...
    if mode == "tui":
        from .tui.app import NovaTuiApp
